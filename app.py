...
def init_db():
    ...
    c.execute("CREATE INDEX IF NOT EXISTS idx_code ON customers(customer_code)")
    conn.commit()
    conn.close()
...
def generate_customer_code():
    ...
    if last:
        last_num = int(last['customer_code'].replace("CUS", ""))
        new_num = last_num + 1
    else:
        new_num = 1
...
def save_customer(line_user_id, display_name, phone=None):
    """บันทึกลูกค้า - จับแค่ครั้งแรก ไม่ซ้ำ"""
    with db_lock:
        conn = get_db_connection()
        c = conn.cursor()
        
        try:
            # เช็คว่ามี user_id นี้แล้วหรือไม่
            ...
            # ถ้ามี race condition ให้รอแล้วลองใหม่
            time.sleep(0.1)
...
def update_clickup_dropdown_async(customer_code, display_name, clean_name_value):
    """เพิ่มลูกค้าเข้า ClickUp Dropdown (แบบ async เพื่อไม่ block)"""
    def do_update():
        if not CLICKUP_DROPDOWN_FIELD_ID:
            print("⚠️ ไม่ได้ตั้งค่า CLICKUP_DROPDOWN_FIELD_ID")
            return
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"❌ ดึง fields ไม่ได้: {resp.status_code}")
                return
            ...
            if not target:
                print(f"❌ ไม่พบ dropdown field: {CLICKUP_DROPDOWN_FIELD_ID}")
                return
            ...
            update_resp = requests.put(
                ...
                headers=headers,
                json={"type_config": {"options": new_opts}},
                timeout=10
            )
            ...
            if update_resp.status_code == 200:
                ...
                print(f"❌ อัปเดตไม่ได้: {update_resp.text}")
...
<body>
    <div class="container">
        <h1>ค้นหาลูกค้า</h1>
        <input type="text" id="searchInput" placeholder="พิมพ์ชื่อหรือรหัส..." autofocus>
        <div id="results"></div>
    </div>
    <script>
        ...
        searchInput.addEventListener('input', function() {
            ...
            resultsDiv.innerHTML = '<p style="text-align:center;color:#667eea;">กำลังค้นหา...</p>';
                ...
                fetch('/api/search?q=' + encodeURIComponent(keyword))
                    ...
                    .then(data => {
                        if (!data.results || data.results.length === 0) {
                            resultsDiv.innerHTML = '<p style="text-align:center;color:#999;">ไม่พบข้อมูล</p>';
                            return;
                        }
                        let html = '';
                        data.results.forEach(customer => {
                            html += '<div class="customer-card">';
                            html += '<div class="customer-code">' + customer.customer_code;
                            html += '<button class="copy-btn" onclick="copyCode(\'' + customer.customer_code + '\', this)">คัดลอก</button>';
                            html += '</div>';
                    ...
                    .catch(() => {
                        resultsDiv.innerHTML = '<p style="text-align:center;color:#999;">เกิดข้อผิดพลาด</p>';
                    });
            }, 300);
        });
...
def index():
    ...
    <p style="color: #28a745; font-weight: bold;">Server ทำงานปกติ</p>
